...
#screen operations
    def register(self):
        ...
        if self.database.register_user(login, password):
...
class MainScreen(tk.Frame):
    ...
    def build_menu(self):
        menu = tk.Frame(self, bg="#dddddd")
        

        tk.Button(
            menu, text="Profil", width=15,
            command=self.show_profile_view
        ).pack(side="left", padx=10, pady=5)

        tk.Button(
            menu, text="Hello World", width=15,
            command=self.show_hello_view
        ).pack(side="left", padx=10, pady=5)
